...
/**
 * <p>Marks a class as a CORBA Servant Locator and a Spring Bean with singleton scope.  An instance of the
 * annotated class is automatically created and set as the servant locator of the POA encapsulated by the
 * the {@link PoaBean} identified by the {@link #poa()} attribute.</p>
 * <p>The PoaBean's policies must allow the use of a Servant Locator and the annotated class must implement
 * the {@link ServantLocator} interface.</p>
...
@Component
public @interface CorbaServantLocator
{
     ...
     * class is to be registered as a Servant Locator.</p>
     * <p>It is the responsibility of the developer that the POA's policies allow the use of a
     * Servant Locator and that only one Servant Locator be created/registered per POA. If any of
