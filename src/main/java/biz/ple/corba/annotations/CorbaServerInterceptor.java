...
import biz.ple.corba.interfaces.StandaloneServerRequestInterceptor;
import biz.ple.corba.springext.CorbaBeanFactoryPostProcessor;
import biz.ple.corba.springext.CorbaBeanPostProcessor;


/**
 * <p>Marks a class as a CORBA Server Request Interceptor and a Spring Bean with singleton scope.
 * If the {@link #serviceName() serviceName} attribute is non-empty, the annotated class becomes part of a
 * 'service definition' and must must implement the {@link ServiceContextPropagatingServerRequestInterceptor}
 * interface. Otherwise, the annotated class must implement at least the {@link ServerRequestInterceptor}
 * interface; it is, however, more convenient to implement the {@link StandaloneServerRequestInterceptor}
 * interface.</p>
 * <p>An instance of the annotated class is automatically created and registered as server request
 ...
 * {@link #serviceName} attribute, if not blank. A service definition includes an instance of at least
 * one of:</p>
 * <ul>
 * <li>{@link ServiceContextPropagatingClientRequestInterceptor}</li>
 * <li>{@code ServiceContextPropagatingServerRequestInterceptor}</li>
 ...
 * <li>{@code @CorbaServerInterceptor}</li>
...
@Component
public @interface CorbaServerInterceptor
{
    /**
     ...
     * to be registered as a server request interceptor. If this attribute empty/not
     * present, and exactly one OrbBean instance exists within the application context,
     * then the instance will be registered with that unique OrbBean; otherwise, a
